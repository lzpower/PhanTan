...
public class Gui_LoaiSanPham extends JPanel {

    private final LoaiSanPhamService service = ServiceFactory.get(LoaiSanPhamService.class, LoaiSanPhamServiceImpl::new);
    ...
    private JPanel createHeader() {
        ...
        JLabel title = new JLabel("QUẢN LÝ DANH MỤC SẢN PHẨM");
    ...
    private JPanel createFormPanel() {
        ...
        JLabel title = new JLabel("Thông tin danh mục sản phẩm");
        ...
        int row = 1;
        panel.add(label("Mã danh mục:"), gc(row++));
        txtMa = new JTextField();
        txtMa.setEditable(false);
        txtMa.setEnabled(false);
        panel.add(txtMa, gc(row++));

        panel.add(label("Tên danh mục:"), gc(row++));
    ...
    private JPanel createTablePanel() {
        ...
        JLabel title = new JLabel("Danh sách danh mục sản phẩm");
        title.setFont(new Font("Segoe UI", Font.BOLD, 20));
        title.setForeground(UiStyle.PRIMARY);
        tablePanel.add(title, BorderLayout.NORTH);

        model = new DefaultTableModel(new Object[]{"STT", "Mã", "Tên danh mục", "Thao tác"}, 0) {
            @Override
        ...
        TableUtility.addActionColumn(table, 3, "/icon/sua.png", "/icon/xoa.png", new TableUtility.ActionButtonCallback() {
            @Override
            public void onEdit(int modelRow) {
                chonDanhMuc(modelRow);
            }
    ...
    private void loadData(List<LoaiSanPhamDto> items) {
        model.setRowCount(0);
        int stt = 1;
        for (LoaiSanPhamDto item : items) {
            model.addRow(new Object[]{stt++, item.getMaLoaiSanPham(), item.getTenLoaiSanPham(), ""});
        }
    }
    ...
    private void chonDanhMuc(int row) {
    ...
    private void addCategory() {
        try {
            LoaiSanPhamDto dto = new LoaiSanPhamDto(service.nextId(), txtTen.getText().trim());
            service.save(dto);
    ...
    private void updateCategory() {
        try {
            if (txtMa.getText().isBlank()) {
                showError("Chọn một danh mục để cập nhật.");
                return;
            }
            LoaiSanPhamDto dto = new LoaiSanPhamDto(txtMa.getText().trim(), txtTen.getText().trim());
            service.update(dto);
            loadData(service.search(txtSearch.getText()));
            resetForm();
        } catch (Exception ex) {
    ...
    private void xoaDanhMuc(int row) {
        String ma = String.valueOf(model.getValueAt(row, 1));
        int confirm = JOptionPane.showConfirmDialog(this, "Xóa danh mục sản phẩm này?", "Xác nhận", JOptionPane.YES_NO_OPTION);
