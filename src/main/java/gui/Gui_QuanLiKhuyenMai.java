...
public class Gui_QuanLiKhuyenMai extends JPanel {

    private static final DateTimeFormatter DATE_FORMATTER = DateTimeFormatter.ofPattern("dd/MM/yyyy");
    private static final String TEN_REGEX = "^[A-ZÀ-Ỹ][a-zà-ỹ]+(\\s[A-ZÀ-Ỹ][a-zà-ỹ]*)*$";

    private final KhuyenMaiService service = ServiceFactory.get(KhuyenMaiService.class, KhuyenMaiServiceImpl::new);
    ...
    private boolean kiemTraDuLieu() {
        ...
        int giaTri;
        try {
            giaTri = Integer.parseInt(giaTriText);
        } catch (NumberFormatException ex) {
            showError("Giá trị khuyến mãi phải là số nguyên từ 1 đến 99.");
            txtGiaTri.requestFocus();
            return false;
        }
        if (giaTri < 1 || giaTri > 99) {
            showError("Giá trị khuyến mãi phải là số nguyên từ 1 đến 99.");
            txtGiaTri.requestFocus();
            return false;
        }

        LocalDate ngayBatDau = dpNgayBatDau.getDate();
        if (ngayBatDau == null) {
            showError("Vui lòng chọn ngày bắt đầu.");
            dpNgayBatDau.getComponentDateTextField().requestFocus();
            return false;
        }
        if (ngayBatDau.isBefore(LocalDate.now())) {
            showError("Ngày bắt đầu phải >= ngày hiện tại.");
            dpNgayBatDau.getComponentDateTextField().requestFocus();
            return false;
        }

        LocalDate ngayKetThuc = dpNgayKetThuc.getDate();
        if (ngayKetThuc == null) {
            showError("Vui lòng chọn ngày kết thúc.");
            dpNgayKetThuc.getComponentDateTextField().requestFocus();
            return false;
        }
