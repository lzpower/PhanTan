...
public class Gui_ChucVu extends JPanel {

    private final ChucVuService service = ServiceFactory.get(ChucVuService.class, ChucVuServiceImpl::new);
    ...
    private JPanel createFormPanel() {
        ...
        JLabel title = new JLabel("Thong tin chuc vu");
        ...
        int row = 1;
        panel.add(label("Ma chuc vu:"), gc(row++));
        txtMa = new JTextField();
        txtMa.setEditable(false);
        txtMa.setEnabled(false);
        panel.add(txtMa, gc(row++));

        panel.add(label("Ten chuc vu:"), gc(row++));
    ...
    private JPanel createTablePanel() {
        ...
        JLabel title = new JLabel("Danh sách chức vụ");
        title.setFont(new Font("Segoe UI", Font.BOLD, 20));
        title.setForeground(UiStyle.PRIMARY);
        tablePanel.add(title, BorderLayout.NORTH);

        model = new DefaultTableModel(new Object[]{"STT", "Ma", "Ten chuc vu", "Thao tác"}, 0) {
            @Override
    ...
    private void loadData(List<ChucVuDto> items) {
        model.setRowCount(0);
        int stt = 1;
        for (ChucVuDto item : items) {
            model.addRow(new Object[]{stt++, item.getMaChucVu(), item.getTenChucVu(), ""});
        }
    }
    ...
    private void chonChucVu(int row) {
    ...
    private void addRole() {
        try {
            ChucVuDto dto = new ChucVuDto(service.nextId(), txtTen.getText().trim());
            service.save(dto);
    ...
    private void updateRole() {
        try {
            if (txtMa.getText().isBlank()) {
                showError("Chon mot chuc vu de cap nhat.");
                return;
            }
            ChucVuDto dto = new ChucVuDto(txtMa.getText().trim(), txtTen.getText().trim());
            service.update(dto);
            loadData(service.search(txtSearch.getText()));
            resetForm();
        } catch (Exception ex) {
    ...
    private void deleteRole() {
        if (txtMa.getText().isBlank()) {
            showError("Chon mot chuc vu de xoa.");
            return;
        }
        int confirm = JOptionPane.showConfirmDialog(this, "Xoa chuc vu nay?", "Xac nhan", JOptionPane.YES_NO_OPTION);
        if (confirm == JOptionPane.YES_OPTION) {
            service.delete(txtMa.getText().trim());
            loadData(service.search(txtSearch.getText()));
            resetForm();
        }
    }

    private void xoaChucVu(int row) {
